...
TFont* Fonts_New (int interspace, int space, char *filename, char *filename_nfo)
{
    ...
    self->fonts_info = calloc (256, sizeof (struct fonts_info));
...
//        printf ("IMG Error:: %s\n", IMG_GetError ( ));
    ...
    for (i=0; i<256; i++)
    {
        ...
        glBegin (GL_QUADS);
         glTexCoord2f ((GLfloat)self->fonts_info[i].x0/(GLfloat)img->width, (GLfloat)self->fonts_info[i].y1/(GLfloat)img->height);
         glVertex2i   (0, 0);
        
         glTexCoord2f ((GLfloat)self->fonts_info[i].x1/(GLfloat)img->width, (GLfloat)self->fonts_info[i].y1/(GLfloat)img->height);
	 glVertex2i   (width, 0);

         glTexCoord2f ((GLfloat)self->fonts_info[i].x1/(GLfloat)img->width, (GLfloat)self->fonts_info[i].y0/(GLfloat)img->height);
	 glVertex2i   (width, height);

         glTexCoord2f ((GLfloat)self->fonts_info[i].x0/(GLfloat)img->width, (GLfloat)self->fonts_info[i].y0/(GLfloat)img->height);
 	 glVertex2i   (0, height);
        glEnd ( );
